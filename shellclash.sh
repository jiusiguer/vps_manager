...
if [ -z "$BASH_VERSION" ]; then
	echo -e "\033[31m检测到非bash环境！正在切换到bash执行...\033[0m"
	if command -v bash >/dev/null 2>&1; then
		exec bash "$0" "$@"
	else
		echo -e "\033[31m系统中未找到bash！请先安装bash后再运行此脚本！\033[0m"
		exit 1
	fi
fi
...
setdir() {
	set_usb_dir() {
		...
		if [ -z "$dir" ] || [ -z "$num" ]; then
			$echo "\033[31m输入错误或为空！请重新设置！\033[0m"
			set_usb_dir
		fi
	}
	set_asus_dir() {
	...
	set_cust_dir() {
		...
		read -p "请输入自定义路径 > " dir
		if [ -z "$dir" ]; then
			$echo "\033[31m输入为空！请重新设置！\033[0m"
			set_cust_dir
		elif [ "$(dir_avail $dir)" = 0 ] || [ -n "$(echo $dir |grep -E 'tmp|opt|sys')" ]; then
...
if [ -n "$CRASHDIR" ]; then
	...
	if [ "$res" = "1" ]; then
		...
		$echo "\033[31m输入错误！已取消安装！请重新运行脚本！\033[0m"
		exit 1
	fi
else
	setdir
